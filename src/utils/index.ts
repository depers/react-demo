...
// 格式化日期
export const formatLocalDate = (date?: Date, rule?: string) => {
  let curDate = new Date()
  if (date) curDate = date
  if (rule === 'yyyy-MM-dd') return curDate.toLocaleDateString().replace('/', '-')
  if (rule === 'HH:mm:ss') return curDate.toLocaleTimeString().replace('/', '-')
